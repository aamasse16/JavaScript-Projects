...
const Calculator = {
    ...
    Display_Value: "0",
    //this will hold the first operand any expressions, we set it to null for now
    First_Operand: null,
...
function Input_Digit(dot) {
    ...
    if (!Calculator.Display_Value.includes(dot)) {
        //we are saying that if the Display_Value does not contain a deicmal point
...
function Handle_Operator(Next_Operator) {
    ...
    Calculator.operator = Next_Operatorr
}
...
keys.addEventListener('click', (event) => {
    ...
    //ensure that AC clears all inputs from the calc screen
